...
import {batchParseScripts, parseScript} from "./util/parseScript";
import {batchBuildChains, buildChainFromFile} from "./util/buildMarkovChain";
import {Bot} from "./bot";
...
(async () => {
    try {
        // await batchBuildChains();
        // await buildChainFromFile('data/speakers/MURPH.txt');
        const murphChainObject = await loadFile('data/chains/JAKE.json');
        const murphBot = new Bot(JSON.parse(murphChainObject));

        Array(50).fill(1).forEach(() => {
            console.log(murphBot.createSentence());
        });
        process.exit(0);
    } catch (e) {
        console.error(e);
